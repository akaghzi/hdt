class ContractorsController < ApplicationController
  ...
  def index
    if params[:search]
      @contractors = Contractor.where(user_id: current_user.id).search(params[:search]).order("name")
    else
  ...
  private
    ...
    def set_contractor
      @contractor = Contractor.find_by(id: params[:id], user_id: current_user.id)
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def contractor_params
      params.require(:contractor).permit(:user_id, :name, :phone, :fax, :email, :description)
    end
end
